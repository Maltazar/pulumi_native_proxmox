...
extract_config_keys() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$in_config" == true ]]; then
            ...
            # Match both namespaced and top-level configuration keys
            # First try to match namespaced keys (section:key)
            if [[ "$line" =~ ^[[:space:]]+([a-zA-Z0-9_]+:[a-zA-Z0-9_]+):[[:space:]]+(.*) ]]; then
                local key="${BASH_REMATCH[1]}"
                local value="${BASH_REMATCH[2]}"
                
                # Uncomment for debugging
                # echo "DEBUG: Found namespaced config key: $key"
                ...
                if [[ "$key" =~ password|token|key|secret ]]; then
                    ...
                    # Check for the secret property in the next few lines
                ...
                # Uncomment for debugging
                # echo "DEBUG: Added namespaced config key: $key (Secret: $is_secret_value, Boolean: $is_boolean_value)"
            
            # Then try to match top-level keys
            elif [[ "$line" =~ ^[[:space:]]+([a-zA-Z0-9_]+):[[:space:]]+(.*) ]]; then
                local key="${BASH_REMATCH[1]}"
                local value="${BASH_REMATCH[2]}"
                
                # Uncomment for debugging
                # echo "DEBUG: Found top-level config key: $key"
                ...
                if [[ "$key" =~ password|token|key|secret ]]; then
                    ...
                    if [ -n "$line_num" ]; then
                        for i in $(seq 1 3); do
                            ...
                            if [[ "$check_line" =~ ^[[:space:]]+[a-zA-Z0-9_]+: || "$check_line" =~ ^[a-zA-Z] ]]; then
                                break
                            fi
                        done
                    fi
                ...
                # Uncomment for debugging
                # echo "DEBUG: Added top-level config key: $key (Secret: $is_secret_value, Boolean: $is_boolean_value)"
            fi
        fi
    done < "Pulumi.yaml"
